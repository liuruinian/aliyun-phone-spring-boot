...
import io.github.liuruinian.phone.store.report.SecretRingReport;
import io.github.liuruinian.phone.store.report.SecretRingReportStore;
 ...
 */
@Slf4j
public class SecretRingReportListener implements MessageListener, ApplicationContextAware {
    ...
    public boolean dealMessage(Message message) {
        ...
        // parameters reference: https://help.aliyun.com/document_detail/400453.html
        ...
        // build SecretRingReport
        SecretRingReport secretRingReport = SecretRingReport.builder()
                .phoneNo(phone_no)
                .secretNo(secret_no)
                .peerNo(peer_no)
                .callType(call_type)
        ...
        // get bean: SecretRingReportStore
        try {
            SecretRingReportStore store = context.getBean(SecretRingReportStore.class);
            boolean added = store.addSecretRingReports(Collections.singleton(secretRingReport));
            if (added) {
                if (log.isInfoEnabled()) {
                    log.info("a secret ring report added.");
                }
            }
        } catch (BeansException e) {
            if (log.isErrorEnabled()) {
                log.error("store secret ring report failed, no bean found: SecretRingReportStore.");
            }
        }
